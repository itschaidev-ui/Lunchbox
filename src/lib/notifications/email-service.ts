...
class EmailService {
  ...
  private initializeTransporter() {
    ...
    const user = process.env.GMAIL_USER || process.env.EMAIL_USER || 'notifications@lunchbox-ai.com';
    const password = process.env.GMAIL_APP_PASSWORD || process.env.EMAIL_PASSWORD;
    ...
    // Try to create transporter with first available service
    for (const config of emailConfigs) {
      try {
        this.transporter = nodemailer.createTransporter(config);
        console.log(`Email service initialized with ${config.service}`);
        break;
      } catch (error) {
    ...
    if (!this.transporter) {
      console.warn('No email service could be initialized');
    }
  }
