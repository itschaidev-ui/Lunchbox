...
export function TaskSummaryDialog({ tasks, onAskAI }: TaskSummaryDialogProps) {
  ...
  const copyAllTasks = () => {
    const allTasksText = [
      ...
      '',
      '🏷️ POPULAR TAGS:',
      tagCounts.slice(0, 10).map(({ tag, count }) => `  • ${tag} (${count})`).join('\n'),
      '',
      '📋 ALL TASKS:',
      '=' .repeat(50),
      '',
    ...
    const categories = [
      { title: '⚠️ OVERDUE', tasks: tasksByStatus.overdue },
      { title: '🕐 DUE TODAY', tasks: tasksByStatus.dueToday },
      { title: '📅 DUE TOMORROW', tasks: tasksByStatus.dueTomorrow },
      { title: '📈 UPCOMING', tasks: tasksByStatus.upcoming },
      { title: '⭕ NO DUE DATE', tasks: tasksByStatus.noDueDate },
